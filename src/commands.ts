...
class CustomCommand {
  ...
  constructor(editor: grapesjs.Editor, opts: PluginOptions = {}){
  ...
  run(editor: grapesjs.Editor, s: any, opts: Record<string, any>): void{
  ...
  stop(editor: grapesjs.Editor, s: any, opts: Record<string, any>) {
    editor.Modal.close();
  }
  ...
  getCodeViewer():CodeMirrorEditor {
    if (!this.codeViewer) {
      this.codeViewer = (this.editor.CodeManager as CodeManagerModule).createViewer({
        codeName: 'htmlmixed',
        theme: 'hopscotch',
        readOnly: 0,
